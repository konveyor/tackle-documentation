...
You can install the {operator} and then install the {project-name} application.
endif::[]

ifeval::["{build}" == "downstream"]
A cluster administrator can install the {operator} on {ocp} 4.7 and later versions.

After the {operator} is installed, a developer can install the {project-name} application in a project.
...
ifeval::["{build}" == "upstream"]
You can install the {operator} by following the procedure on link:https://operatorhub.io[operatorhub.io].
...
. Follow the installation procedure to install the {operator}.
endif::[]
ifeval::["{build}" == "downstream"]
You can install the {operator} by using the OpenShift web console.
