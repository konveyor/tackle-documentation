// common document attributes
include::modules/common-attributes.adoc[]
ifeval::["{build}" == "downstream"]
= {title}
endif::[]
ifeval::["{build}" == "upstream"]
= {title} (draft)
endif::[]
:toc:
:toc-title:
:toclevels: 3
:compat-mode:
:context: pathfinder

ifeval::["{build}" == "downstream"]
include::modules/making-open-source-more-inclusive.adoc[]
endif::[]

include::modules/about-pathfinder.adoc[]
include::modules/installing-pathfinder.adoc[]

include::modules/logging-in-to-pathfinder.adoc[leveloffset=+2]

[id="managing-stakeholders-job-functions-business-services-and-tags_{context}"]
== Managing controls

The *Controls* page of the {project-name} web console enables you to manage the following entities:

Stakeholder::
A stakeholder is an individual with an interest in an application or a subject matter expert. Stakeholders are contributors to the application assessment process.

Stakeholder group::
A stakeholder group is a group of related stakeholders. A stakeholder can belong to one or more stakeholder groups. Stakeholder groups are used to assign multiple stakeholders to review an assessment.

Job function::
A job function is an attribute of a stakeholder. You use a predefined job function or create a new job function.

Business service::
A business service is an attribute of an application, for example, credit card service, transportation, or IT support. A business service is owned by a stakeholder.

Tag::
A tag is an attribute of an application. Tags are an extensible and flexible way to add metadata to applications. Tags are grouped under a parent tag type. You can use predefined tag types and tags or create new ones.

include::modules/creating-stakeholder.adoc[leveloffset=+2]
include::modules/creating-stakeholder-group.adoc[leveloffset=+2]
include::modules/creating-job-function.adoc[leveloffset=+2]
include::modules/creating-business-service.adoc[leveloffset=+2]
include::modules/creating-tag-type.adoc[leveloffset=+2]
include::modules/creating-tag.adoc[leveloffset=+2]

[id="managing-applications_{context}"]
== Managing applications

You can create, import, tag, and modify your applications in the *Application inventory* page of the {project-name} web console.

include::modules/creating-application.adoc[leveloffset=+2]
include::modules/importing-applications.adoc[leveloffset=+2]
include::modules/managing-imports.adoc[leveloffset=+2]
include::modules/tagging-application.adoc[leveloffset=+2]
include::modules/adding-business-service-to-applications.adoc[leveloffset=+2]
include::modules/managing-application-dependencies.adoc[leveloffset=+2]

[id="managing-assessments_{context}"]
== Managing assessments

You can start, edit, review, and delete application assessments in the *Application inventory* page of the {project-name} web console.

include::modules/starting-assessment.adoc[leveloffset=+2]
include::modules/editing-assessment.adoc[leveloffset=+2]
include::modules/reviewing-assessment.adoc[leveloffset=+2]
include::modules/deleting-assessment.adoc[leveloffset=+2]

include::modules/about-reports.adoc[leveloffset=+1]
